//Cheung Tin Long 19055971d Intellij
import java.awt.*;
import java.awt.event.WindowAdapter;
import java.awt.event.WindowEvent;
import java.io.BufferedWriter;
import java.io.File;
import java.io.FileWriter;
import java.io.IOException;

public class SpecificBookManuel extends Frame {
    private MyLinkedList<Book> bookstore;
    private String SelectedISBN,SelectedTitle;
    private PhotoPanel photoPanel;
    private BookQueuePanel bookQueuePanel;
    private int SelectedIndex;
    public SpecificBookManuel(MyLinkedList<Book> bookstore, String SelectedISBN,String SelectedTitle, int SelectedIndex) {
        this.bookstore=bookstore;
        this.SelectedISBN=SelectedISBN;
        this.SelectedTitle=SelectedTitle;
        this.SelectedIndex=SelectedIndex;
        Book current= bookstore.get(SelectedIndex);
        GridLayout gl = new GridLayout(1, 2, 0, 0);
        setLayout(gl);
        setTitle(SelectedTitle);
        setSize(900, 800);
        setVisible(true);
        addWindowListener(new CloseHandler());
        photoPanel= new PhotoPanel(current);
        add(photoPanel);

        bookQueuePanel = new BookQueuePanel(current);
        add(bookQueuePanel);
    }

    //Closing by clicking x button
    protected  class CloseHandler extends WindowAdapter {
        public void windowClosing(final WindowEvent event) {
            try {
                save();
            } catch (IOException e) {
                e.printStackTrace();
            }
            System.exit(0);
        }
    }
    public void save() throws IOException {
        File libraray = new File("C:\\library");
        if(!libraray.exists()){
            File database = new File("C:\\library\\database");
            File photo = new File("C:\\library\\static");
            database.mkdirs();
            photo.mkdirs();
        }
        Book current = photoPanel.getCurrent();
        current.setAvailable(bookQueuePanel.getAvailable());
        current.setReservedQueue(bookQueuePanel.getReservedQueue());
        bookstore.set(SelectedIndex,current);
        BufferedWriter bufw = new BufferedWriter(new FileWriter(new File("C:\\library\\database\\store.txt")));
        Object[] temp=bookstore.toArray();
        for(int i=0;i<temp.length;++i){
            Book book= (Book) temp[i];
            StringBuilder str= new StringBuilder();
            str.append(book.getISBN());
            str.append("`!");

            str.append(book.getTitle());
            str.append("`!");

            if(book.isAvailable()){
                str.append("1");
            }
            else {
                str.append("0");
            }
            str.append("`!");

            Object[] queue = book.getReservedQueue().getList().toArray();
            if(queue.length==0){
                str.append("e");
                str.append("`!");
            }
            else {
                for(int j=0;j<queue.length;++j){
                    String names= (String) queue[j];
                    str.append(names);
                    if(j != (queue.length-1)){
                        str.append(",");
                    }
                    else{
                        str.append("`!");
                    }
                }
            }
            if(book.getPath().length()==0){
                str.append("e");
            }
            else {
                str.append(book.getPath());
            }
            bufw.write(str.toString());
            bufw.newLine();
        }
        bufw.close();
    }

}
